...
- [x] `firestore_place_repository.dart` — Firestore impl of PlaceRepository (all methods, correct entity mapping)
- [x] `firestore_review_repository.dart` — Firestore impl of ReviewRepository (cursor pagination, sort, likes)
- [x] `firebase_auth_repository.dart` — Firebase Auth impl (OTP flow returns verificationId, batch follow/unfollow, avatar upload via StorageService)
- [x] `firebase_add_place_repository.dart` — submits to `places_pending`, uploads photos via StorageService
- [x] `storage_service.dart` — Firebase Storage upload (single + batch, progress callback)
- [x] All providers switched to conditional Firebase/mock via `AppEnv.useFirebase` flag

### Map Feature
- [x] Map screen — real `GoogleMap` widget when `MAPS_API_KEY` set, illustrated fallback otherwise
...
- [x] Pins driven by `feedPlacesProvider` (real data from Firestore or mock)
- [ ] "Near me" mode (device GPS)
...
- [x] JO3T Score algorithm (Cloud Function — `functions/src/index.ts`, weighted avg + recency + confidence dampener)
...
- [ ] Firebase project setup (dev / staging / prod) — run `flutterfire configure` to generate real config files
- [x] `google-services.json` template + `.gitignore` entry (real file generated by `flutterfire configure`)
- [x] `--dart-define` environment config — `AppEnv.useFirebase`, `AppEnv.mapsApiKey`, `AppEnv.algoliaAppId`
- [x] Firestore security rules — `firestore.rules` (user/place/review/pending access control)
- [x] Firestore composite indexes — `firestore.indexes.json` (8 indexes for all query patterns)
- [x] Firebase Storage rules — `storage.rules` (avatars, place photos, review photos — 5MB limit)
- [x] Algolia index sync Cloud Function — `syncPlaceToAlgolia` in `functions/src/index.ts`
- [x] GitHub Actions CI pipeline — `.github/workflows/ci.yml` (analyze, test, APK build, functions build)
- [x] Firebase App Distribution — `.github/workflows/deploy.yml` (deploy functions + distribute APK)
- [x] Crashlytics + Analytics — initialized in `firebase_service.dart`, wired in `main.dart`
...
- [x] Real push notifications — `FcmService` wired in `main.dart`, foreground via `flutter_local_notifications`, FCM triggers in Cloud Functions (`onReviewCreated`, `onFollowCreated`)
